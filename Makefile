...
IMAGE_REPO ?= registry.cn-shenzhen.aliyuncs.com/mutoulazy
IMAGE_NAME ?= my-minicni
CNI_NAME ?= minicni
...
build-image: build
	...
	@docker build -t $(IMAGE_REPO)/$(IMAGE_NAME):$(RELEASE_VERSION) -f deployments/Dockerfile .
